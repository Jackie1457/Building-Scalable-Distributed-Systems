...
public class MultiThreadsTest {
  ...
  private final static int TOTAL_REQUESTS_500K = 1000;
//  private final static int TOTAL_REQUESTS_500K = 1000;

//  private final static int PER_THREAD_REQUESTS = 10;
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    CountDownLatch postingThreadsStarted = new CountDownLatch(NUM_THREADS);
    for (int i = 0; i < NUM_THREADS; i++){
      ConsumerSwipeApi consumerSwipeApi = new ConsumerSwipeApi(buffer, result, completed, TOTAL_REQUESTS_500K/NUM_THREADS, postingThreadsStarted);
      Thread consumerThreads = new Thread(consumerSwipeApi);
      consumerThreads.start();
    }
    
    GetThread getThread = new GetThread(completed, postingThreadsStarted);
    Thread getThreadInstance = new Thread(getThread);
    getThreadInstance.start();

    completed.await(); // wait until all threads complete
    getThread.terminate(); // terminate the GetThread
